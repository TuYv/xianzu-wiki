...
// 隔离页面逻辑:把 FamilyTree 桩成只回显收到的 focusId / depth
vi.mock('../components/FamilyTree', () => ({
  FamilyTree: ({ focusId, depth }: { focusId: number; depth: number }) => (
    <div data-testid="tree">
      focus={focusId} depth={depth}
    </div>
  ),
}));
...
describe('TreePage', () => {
  ...
  it('“查看整族”切换把 depth 放大,再次点击回到中心视图', async () => {
    mockList.mockResolvedValue(chars);
    mockRels.mockResolvedValue([]);
    renderAt('1');
    ...
    expect(screen.getByTestId('tree').textContent).toContain('depth=999');
    // 按钮文案翻转,可切回
    const back = screen.getByRole('button', { name: '回到以当前人物为中心' });
    await userEvent.click(back);
    expect(screen.getByTestId('tree').textContent).toContain('depth=3');
  });
